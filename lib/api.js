...
export async function getCategoryNameFromSlug(slug) {
  ...
  return data?.categories.edges[0].node.name
}

export async function getMenuFromSlug(slug) {
  const data = await fetchAPI(`
      query ($slug: String!) {
...
export async function getMorePostsByCategory(slug, batchSize, endCursor) {
  const data = await fetchAPI(
    `
    query ($slug: String!, $batchSize: Int, $endCursor: String) {
