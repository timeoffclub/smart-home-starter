...
export async function getPropsForCategory(slug) {
    const data = await fetchAPI(`
    query ($slug: String!) {
...
export async function getFeaturedIdsWithSlug(first, after, slug) {
    const data = await fetchAPI(`
        query ($first: Int, $after: String, $slug: String) {
            posts(first: $first, after: $after, where: {categoryName: "featured"}) {
                nodes {
