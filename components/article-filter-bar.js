                    ...
                    <div className='inline-block'>
                        ...
                        <div className={moreTabs ? 'absolute block bg-white z-10' : 'hidden absolute bg-white z-50'}>
                            {myMenu.slice(tabsPaginate).map((tab) => (
                                tab.label !== myCategory &&
                                <div onClick={() => {toggleMoreTabs(); onFilter(tab.label); setActiveTab(tab.label)}} key={tab.label} className={activeTab !== tab.label ? `block font-semibold text-xl tracking-wider px-5 py-1 cursor-pointer` : `block font-semibold text-xl text-white bg-black tracking-wider px-5 py-1 cursor-pointer`}>
                                    {tab.label}
                                </div>
                            ))}
                        </div>
