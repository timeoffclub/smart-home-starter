            ...
            {!featuredArticle ?
                ...
                <div className='container mb-12'>
                    ...
                    <div className='hidden sm:grid px-5 sm:px-0 md:px-6 xl:px-0 sm:grid-cols-2 lg:grid-cols-4 lg:min-h-[300px]  gap-5 mt-5'>
                        ...
                        <div  key='ad' className='bg-gray-200 h-72 sm:row-start-1 sm:row-span-3 sm:col-span-1 sm:col-start-2 lg:row-start-1 lg:row-span-3 lg:col-start-3 lg:col-span-2 xl:h-auto xl:col-span-1 xl:row-start-1 xl:row-span-3 xl:col-start-4 mt-10 md:mt-0'>
                            AD
                        </div>
                    </div>
                            ...
                            <div key={el.id} className='flex items-center mb-3 lg:mb-0'>
                                ...
                                <div className='ml-2'>
                                    ...
                                    <div className='text-smart-blue text-base font-medium uppercase tracking-wider'>
                                        {el.categories.edges.filter((e) => e.node.name !== myCategory && e.node.name !== 'Featured').map((cat, index) => (
                                            <span key={cat.node.id}>
                                                <a className='text-smart-blue font-semibold hover:text-smart-green' href={`../category/${cat.node.slug}`}>
                                                    {cat.node.name}
                                                </a>
                                ...
                                <div key={el.id} className='col-span-2  sm:col-span-1 inline-flex flex-wrap sm:w-72 md:w-full mb-5'>
                                    ...
                                    <div className='border-l-4 border-l-smart-blue/0'>
                                        ...
                                        <div className='text-base text-smart-blue'>
                                            {el.categories.edges.filter((e) => e.node.name !== myCategory && e.node.name !== 'Featured').map((cat, index) => (
                                                <span key={cat.node.id}>
                                                    <a className='text-smart-blue font-semibold' href={`../category/${cat.node.slug}`}>{cat.node.name}</a> {index < (el.categories.edges.filter((e) => e.node.name !== myCategory && e.node.name !== 'Featured').length - 1) ? <span>| </span> : <span></span>}
                                                </span>
                                            ))}
                                        </div>
                                    </div>
